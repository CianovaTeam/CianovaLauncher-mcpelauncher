    ...
    if name:
        ...
        if name:
            ...
            if name not in profiles:
                ...
                from src.core.app_logic import switch_profile
...
def delete_profile(app, name):
    if name == c.UI_PROFILE_DEFAULT or name == app.config.get(c.CONFIG_KEY_CURRENT_PROFILE):
        return False
...
def rename_profile(app, old, new):
    if old == c.UI_PROFILE_DEFAULT or not new:
        return False
